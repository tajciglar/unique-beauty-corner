...
const ViewAppointment: React.FC<ViewAppointmentProps> = ({ appointment, onClose, onDelete }) => {
  ...
  return (
    <div className="fixed z-10 top-0 left-0 w-full h-full flex items-center justify-center bg-[rgba(0,0,0,0.5)]">
      <div className="absolute bg-background bg-opacity-90 p-4 rounded-lg text-base flex flex-col gap-4">
        ...
        <button className="bg-red-500 hover:bg-red-600 hover:text-white text-white py-1 px-3 rounded-lg" onClick={() => onDelete(appointment.id)}>
          Izbriši termin
        </button>
        <button className=" text-white py-1 px-3 rounded-lg" onClick={onClose}>
          Zapri
        </button>
      </div>
    </div>
